...
- **Repoint modele semantique apres CLONE prod (L146)** : dupliquer un projet DSS ne reecrit PAS les refs dataset des modeles semantiques (`datasetRef` logique `<KEY>.Dataset` ET table physique `<KEY>_dataset` des golden queries restent sur la cle SOURCE -> le SQL tape la table DEV). L'UI ne le corrige pas. Depuis un notebook DU CLONE : `dataiku-agents/OWISMIND/repoint_tickets_prod_clone.py` (repoint pur) ou `add_solution_and_repoint_prod_clone.py` (repoint + restaure la colonne `Solution`). Remap des 2 formes `<KEY>.` et `<KEY>_`, cle cible = projet courant, modele resolu par nom, DRY_RUN puis re-index. Re-ajouter une colonne retiree = inverse exact de `drop_column_and_reindex.py`.
