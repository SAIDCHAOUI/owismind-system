...
> **OWIsMind (3.9) :** la régression côté backend = `unittest` stdlib (des centaines de cas), avec **LLM mocké** dans le *scaffolding* déterministe (loop caps, extraction, rendering, paramétrage) + **golden queries** depuis `docs/questions_asked.md` + **tests anti-dérive** (`KNOWN_TOOL_NAMES`/`KNOWN_BLOCK_IDS` ↔ registre). Lancement : `python3 -m unittest discover -s OWIsMind_PRD_V1_3_DEV/tests`.
