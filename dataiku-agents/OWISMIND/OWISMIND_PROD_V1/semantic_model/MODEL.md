> PROD copy. Live model `Drive_Revenues_Model` (id `a7K9jYk`) in project
> `OWISMIND_PROD_V1`, queried by `revenue_semantic_query` (`sgk5pfln`).
> Promoted from DEV; iterate via this folder's scripts.

# Drive_Revenues_Model - what the SQL brain actually contains
...
sub-agent SalesDrive_revenue_expert
   COMPOSE: build a grounded natural-language question (exact values, scenario, period)
       │
       ▼ project.get_agent_tool("sgk5pfln").run({"question": "..."})
   revenue_semantic_query  (Semantic Model Query tool, id sgk5pfln)
       │  writes AND runs the SQL using the semantic model below
       ▼
   Drive_Revenues_Model  (this model, version v1 / Active)
       │
       ▼
   PostgreSQL  →  OWISMIND_PROD_V1.DRIVE_Revenues   (read as the calling user)
```
...
| Setting | Value |
|---|---|
| Tool name / id | `revenue_semantic_query` / `sgk5pfln` |
| Project | `OWISMIND_PROD_V1` |
| Semantic Model | `Drive_Revenues_Model` |
...
(`OWISMIND_PROD_V1.DRIVE_Revenues`). They are a documentation lens over one
...
- The aligned model was built from a **read-only copy** of the old model
  ...
  tool. The live active model is named `Drive_Revenues_Model`.
...
- To **refresh this snapshot**: run `dump_semantic_model.py` (exports the live
  `get_raw()` to `Drive_Revenues_Model.v1.json`).
