# semantic_model/ - the SQL brain that revenue_semantic_query queries (PROD)
...
> (`sgk5pfln`), called by the sub-agent, points at the model documented here.

This folder holds the **scripts** that iterate the aligned semantic model in the PRODUCTION
project, plus a human-readable snapshot of its config. The live model itself lives in DSS.
Development happens in `../../OWISMIND_DEV/semantic_model/`; changes are promoted here with the
PROD ids (`tools/promote_agents_to_prod.py` regenerates `update_aligned_semantic_model.py`).
...
| `update_aligned_semantic_model.py` | In-place MODIFY of the aligned model: refresh instructions + golden queries on the active version (no create, no re-index). The going-forward iteration path. Regenerated from the DEV copy by the promotion script (PROD ids baked in). |
| `dump_semantic_model.py` | Export the live model `get_raw()` to `Drive_Revenues_Model.v1.json` (refresh the snapshot, no transcription drift). |
...
Not in this folder (by design): `build_aligned_semantic_model.py` (one-time CREATE, ran in DEV)
and the tickets scripts live in `../../OWISMIND_DEV/semantic_model/`; the cross-project utilities
`migrate_semantic_model_to_project.py` / `remap_semantic_model.py` live in `../../` (OWISMIND/).

> **The model config is versioned here as JSON.** The model's full `get_raw()` config lives in
> `Drive_Revenues_Model.v1.json`. Paste a fresh `dump_semantic_model.py` output into it after
> every model change, so the config is in the repo and never needs pasting in chat. The canonical
> SQL-generation rules live verbatim in `update_aligned_semantic_model.py` (`NEW_INSTRUCTIONS`,
> byte-identical to the DEV copy apart from the per-project ids).

## Live model and tool (source of truth = DSS)

- Active model: **`Drive_Revenues_Model`** (`a7K9jYk`), project `OWISMIND_PROD_V1`. Carries the
  same aligned brain as the validated DEV model (`Drive_Revenues_Semantic_Model` in
  `OWISMIND_DEV`); refresh it by running `update_aligned_semantic_model.py` here after each
  promotion.
- Bound to the tool **`revenue_semantic_query`** (`sgk5pfln`): LLM `vertex_ai/claude-sonnet-4-6`,
  embedding `vertex_ai/text-embedding-005`, **Agent mode OFF** (the faster linear SQL pipeline,
  not a multi-step agent), access datasets as the calling user, project `OWISMIND_PROD_V1`.
- Queried by the sub-agent `SalesDrive_revenue_expert` (`agent:uO5hEzAs`) via
  `SEMANTIC_TOOL_ID = "sgk5pfln"` when `SQL_ENGINE = "semantic_tool"` (the default).

## What the aligned rebuild fixed (vs the old DEV model `2O2KcHw`)
...
## How to iterate (PROD = promotion target, never develop here)

The aligned model already exists and `revenue_semantic_query` is already bound to it; the steps
below apply AFTER a change was validated in DEV and promoted to this folder.

1. **Promote the rules.** Iterate `NEW_INSTRUCTIONS` / `GOLDEN_QUERIES` in the DEV copy, validate
   in DEV, then run `python3 tools/promote_agents_to_prod.py` (repo) to regenerate this folder's
   `update_aligned_semantic_model.py` with the PROD ids. Run it in a DSS notebook (project
   OWISMIND_PROD_V1). In place, no re-index.
...
3. **Refresh the repo snapshot**: run `dump_semantic_model.py` and commit
   `Drive_Revenues_Model.v1.json` + update `MODEL.md` if the structure changed.
...
5. **No sub-agent re-paste needed for a model-only change** (the sub-agent references the tool by
   id). Re-paste `SalesDrive_revenue_expert.py` into its Code Agent (`agent:uO5hEzAs`, env 3.11)
   only when the agent code itself changes.

## The tickets semantic model (second domain)

The tickets domain is **not in PROD yet**: its model, tool and Code Agent are being finished and
validated in `OWISMIND_DEV` (see `../../OWISMIND_DEV/semantic_model/` and
[`../../../PLAYBOOK_ADD_AGENT.md`](../../../PLAYBOOK_ADD_AGENT.md)). Once validated, it will be
promoted here with PROD ids (own model + own `tickets_semantic_query` tool: a shared model is
forbidden by the one-table-never-JOIN rule and the 1:1 tool-to-model binding). Until then the
PROD orchestrator answers tickets questions with the honest capability gap.
...
- The model's instructions and golden queries live in git history: check out the previous
  version of `update_aligned_semantic_model.py` and re-run it in the PROD notebook (in place).
  The sub-agent code also lives in git; revert and re-paste. The profile overrides can be removed
  by editing the overrides dataset and re-running the profile recipe.
