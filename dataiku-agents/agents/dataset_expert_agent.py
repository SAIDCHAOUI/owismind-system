...
#   - STANDALONE file: stdlib + dataiku only. Pasted into a DSS Code Agent.
# =============================================================================
...
class MyLLM(BaseLLM):
    ...
    def _call_json_llm(self, project, system_prompt, user_msg, schema, span):
        """2 attempts (native JSON mode then prompt-only) -> parsed dict|None."""
    ...
    def process_stream(self, query, settings, trace):
        ...
        try:
            ...
            if u["scope"] == "out_of_scope":
                ...
                yield {"chunk": {"text": OUT_OF_SCOPE_TEXT[lang].format(
            ...
            if u["intent"] == "about_data":
                yield _block("about_data")
                yield {"chunk": {"text": build_about_answer(profile, lang)}}
                yield _agent_result("ready", u)
                return

            # ============ RESOLVE (value grounding) ============
            resolutions, filters = [], []
            if u["terms"]:
                ...
                yield _tool_start("resolve_filter_value")
                with trace.subspan("dataset-expert:resolve-values") as sp:
                    sp.inputs["raw_values"] = base_terms
                    try:
                        resolutions = self._resolve_terms(profile, base_terms, trace)
                    except Exception as e:
                        ...
                        yield _agent_result("error", u)
                        return
                    # qualified-term column preference + ambiguity policy
                    refined = []
                ...
                if any(r.get("status") in ("ambiguous", "unresolved")
                    ...
                    yield _block("clarify_user")
                    yield {"chunk": {"text": clarification or
            ...
            resolved_filters = resolved_filters_summary(resolutions)
            table = self._get_table(profile.dataset_name)

            # ============ BUILD SQL + EXECUTE (with repair for custom) =========
            yield _block("run_sql")
            ...
            if u["intent"] != "custom":
                try:
                    ...
                    with trace.subspan("semantic-model-query") as sp:
                        sp.outputs["sql"] = sql
                        try:
                            columns, rows = self._run_sql(profile.dataset_name, sql)
            ...
            if u["intent"] == "custom":
                ...
                for attempt in range(1, MAX_CUSTOM_SQL_ATTEMPTS + 1):
                    ...
                    with trace.subspan("semantic-model-query") as sp:
                        ...
                        sp.attributes["attempt"] = attempt
                        try:
                            self._explain(profile.dataset_name, sql)
                            columns, rows = self._run_sql(profile.dataset_name, sql)
                            result = shape_result(columns, rows)
                            sp.outputs["success"] = True
                            sp.outputs["row_count"] = len(result["rows"])
            ...
            if not result or not result.get("rows"):
                ...
                if tm and u["period"]["mode"] == "explicit" and (tm.get("min")
                    ...
                    hint += NO_DATA_HINT_PERIOD[lang].format(min=tm.get("min"),
                                                             max=tm.get("max"))
                yield {"chunk": {"text": NO_DATA_TEXT[lang] + hint}}
                yield _agent_result("no_data", u,
                                    resolved_filters=resolved_filters,
            ...
            yield _agent_result("ready", u, resolved_filters=resolved_filters,
                                ...
                                row_count=len(result["rows"]),
                                attempts=len(executed))
            logger.info("Dataset expert done in %dms",
                        int((time.perf_counter() - t0) * 1000))

        except Exception:
            logger.exception("Dataset expert failure")
            lang = (u or {}).get("language", "fr")
            yield {"chunk": {"text": INTERNAL_ERROR_TEXT.get(lang,
                                                             INTERNAL_ERROR_TEXT["fr"])}}
            yield _agent_result("error", u)
