# =============================================================================
# OWIsMind — ORCHESTRATEUR v3.0 "Le Cerveau" (Code Agent Dataiku)
# -----------------------------------------------------------------------------
...
#
# v3.0 (dataiku-agents — generic dataset experts + parallel fan-out):
#   - New capability flavour: GENERIC Dataset Expert code agents (repo file
#     dataiku-agents/agents/dataset_expert_agent.py — one DSS Code Agent per
#     dataset, same file, different CONFIG block). Their blockIds/tool names
#     are labeled by the registry like any other sub-agent.
#   - PARALLEL EXECUTE: multi-step plans fan out the agent steps on a bounded
#     thread pool (MAX_PARALLEL_AGENTS). Worker threads stream each sub-agent
#     and push displayable events into a queue; the main generator drains and
#     re-yields them LIVE (the timeline shows agents progressing together).
#     Trace spans are created post-hoc on the main thread (SpanBuilder is not
#     assumed thread-safe); usage accumulation is main-thread only. A 360
#     analysis = one plan step per staffed domain, now wall-clocked at the
#     SLOWEST source instead of the sum.
#   - Mono-step behaviour, relay path, events and Evidence capture: UNCHANGED.
#
...
MAX_PARALLEL_AGENTS = 3            # v3: fan-out borné des étapes agent (sécurité instance)
PARALLEL_TOTAL_TIMEOUT_S = 600     # v3: garde-fou global du drain parallèle
...
CAPABILITIES = {
    ...
    # OWN read-only SQL from the dataset profile + value index (no semantic
    # model tool). To activate: create the DSS Code Agent, paste the file,
    # put its agent id below, set THIS entry enabled=True and salesdrive_v2
    ...
    "revenue_expert": {
        ...
        "planner_description": (
            ...
            "product, solution, solution line, sirano product, partner, "
            "distribution type, sales entity, sales zone, parent group, month or "
            "year; totals, top-N rankings, shares of total, period comparisons, "
            "actuals-vs-budget deltas and variance, trends, YTD, distinct-value "
            "lists and counts; can also DESCRIBE the data it covers. Handles "
            "multi-period and multi-phase comparisons WITHIN ONE single step. "
            "Data source: DRIVE_Revenues. You do NOT pre-judge what this data "
            "contains — route the question; only this agent can confirm or deny "
            "a specific figure."
        ),
        "dataset_sources": [
            "https://dataiku-datalab-owi.authidh.itn.intraorange/workspaces/OWISMIND/OWISMIND/DATASET/DRIVE_Revenues",
        ...
        "dataset_label_en": "OWI customer revenue base (DRIVE_Revenues)",
        "dataset_ref": {"project_key": "OWISMIND", "dataset_name": "DRIVE_Revenues"},
        # blockIds emitted by dataset_expert_agent.py (FROZEN contract there).
        "block_labels": {
            "resolve":          {"fr": "analyse de la question et identification des filtres",
                                 "en": "analyzing the question and resolving filters"},
            "run_sql":          {"fr": "interrogation de la base de données",
                                 "en": "querying the database"},
            "format_output":    {"fr": "mise en forme des résultats",
                                 "en": "formatting the results"},
            "clarify_user":     {"fr": "demande de précision",
                                 "en": "asking for clarification"},
            "out_of_scope_msg": {"fr": "question hors périmètre",
                                 "en": "out-of-scope notice"},
            "about_data":       {"fr": "présentation des données disponibles",
                                 "en": "describing the available data"},
        },
        "tool_labels": {
            "resolve_filter_value": {"fr": "résolution des noms (clients, produits…)",
                                     "en": "resolving names (customers, products…)"},
            "dataset_sql_query":    {"fr": "génération et exécution de la requête SQL",
                                     "en": "generating and running the SQL query"},
        },
        "pass_context": True,
...
class MyLLM(BaseLLM):
    ...
    def process_stream(self, query, settings, trace):
        ...
        try:
            ...
            # v3: PARALLEL fan-out when the plan has 2+ agent steps. Multi-step
            # plans never relay text live (relay_text is mono-step only), so
            # interleaved events are safe for the timeline (stepIndex keys
            # every event). Single-agent plans keep the live relay path below.
            n_agent_steps = sum(1 for s in steps if s["kind"] == "agent")
            if not relay_text and n_agent_steps > 1:
                results = yield from self._execute_steps_parallel(
                    project, steps, caps, trace, lang, sub_context,
                    total_usage, step_count)
            else:
    ...
    # --------------------------------------------- v3: PARALLEL agent fan-out
    def _execute_steps_parallel(self, project, steps, caps, trace, lang,
                                sub_context, total_usage, step_count):
        """Multi-step EXECUTE with bounded parallel sub-agent fan-out.

        Tool steps run inline first (fast, deterministic order). Agent steps
        are submitted to a thread pool; each worker streams its sub-agent and
        pushes displayable events into a queue, which this generator drains
        and re-yields LIVE. All trace spans, usage accumulation and SQL
        tagging happen on the MAIN thread after each step completes
        (SpanBuilder/total_usage are not assumed thread-safe). Returns the
        results list aligned with the steps order."""
        results = [None] * len(steps)
        out_q = queue.Queue()

        agent_items = []   # (idx0, idx, step, cfg, label)
        for idx0, step in enumerate(steps):
            idx = idx0 + 1
            cfg = caps[step["capability"]]
            label = cfg.get("label_%s" % lang, step["capability"])
            if step["kind"] == "tool":
                ...
                agent_items.append((idx0, idx, step, cfg, label))

        by_idx = {}
        for idx0, idx, step, cfg, label in agent_items:
            by_idx[idx] = (idx0, step, cfg, label)
            yield {"chunk": {"text": STEP_PROGRESS[lang].format(label=label)}}
            # ORCH-04: no agentId in eventData (id stays in the step span).
            yield _ev_l("CALLING_AGENT", lang, {
                "agentKey": step["capability"], "question": step["instruction"],
                "stepIndex": idx, "stepCount": step_count}, label=label)

        pool = ThreadPoolExecutor(max_workers=min(MAX_PARALLEL_AGENTS,
                                                  max(1, len(agent_items))))
        try:
            for idx0, idx, step, cfg, label in agent_items:
                pool.submit(self._run_agent_step_blocking, project, idx, step,
                            cfg, lang, sub_context, out_q)

            pending = len(agent_items)
            ...
            while pending > 0 and time.monotonic() < deadline:
                ...
                if kind == "event":
                    yield payload
                    continue
                # kind == "done": finalize this step on the main thread.
                pending -= 1
                idx0, step, cfg, label = by_idx[idx]
                res = payload["res"]
                sub_trace = payload.get("sub_trace")
                with trace.subspan("step_%d:agent:%s" % (idx, step["capability"])) as sp:
                    sp.attributes["agentId"] = cfg["agent_id"]
                    sp.attributes["parallel"] = True
                    sp.attributes["durationMs"] = res["duration_ms"]
                    sp.inputs["question"] = step["instruction"]
                    sp.outputs["answer"] = res["output"]
                    if payload.get("error"):
                        sp.attributes["error"] = payload["error"]
                    if sub_trace:
                        try:
                            sp.append_trace(sub_trace)
                        except Exception as e:
                            logger.exception("append_trace failed for step %d", idx)
                            sp.attributes["trace_append_error"] = str(e)[:300]
                res["usage"] = _sum_usage(_find_usage_metadata(sub_trace)) if sub_trace else {}
                generated_sql = _find_generated_sql(sub_trace) if sub_trace else []
                for n, item in enumerate(generated_sql, start=1):
                    item["sql_id"] = "s%dq%d" % (idx, n)
                    item["step_index"] = idx
                    item["agent_key"] = step["capability"]
                res["generated_sql"] = generated_sql
                _acc_usage(total_usage, res.get("usage"))
                if res["status"] == "error":
                    yield _ev_l("ERROR", lang, {
                        "stage": "agent_step", "stepIndex": idx,
                        "agentKey": step["capability"],
                        "message": ERROR_CODE_AGENT_STEP_FAILED})
                yield _ev_l("AGENT_DONE", lang, {
                    "agentKey": step["capability"], "stepIndex": idx,
                    "status": res["status"], "durationMs": res["duration_ms"],
                    "usage": res.get("usage"),
                    "generatedSql": res.get("generated_sql", []),
                    "agentResult": res.get("agent_result")}, label=label)
                tmpl = STEP_OK if res["status"] == "ok" else STEP_FAIL
                yield {"chunk": {"text": tmpl[lang].format(
                    label=label, secs=round(res["duration_ms"] / 1000.0, 1))}}
                results[idx0] = res

            # Deadline hit with steps still pending: mark them failed honestly
            # (their threads keep finishing in the background, harmless).
            for idx, (idx0, step, cfg, label) in by_idx.items():
                if results[idx0] is None:
                    yield _ev_l("ERROR", lang, {
                        "stage": "agent_step", "stepIndex": idx,
                        "agentKey": step["capability"],
                        "message": ERROR_CODE_AGENT_STEP_FAILED})
                    yield {"chunk": {"text": STEP_FAIL[lang].format(label=label)}}
                    results[idx0] = {
                        "kind": "agent", "capability": step["capability"],
                        "instruction": step["instruction"], "status": "error",
                        "output": "", "duration_ms": PARALLEL_TOTAL_TIMEOUT_S * 1000,
                        "usage": {}, "generated_sql": [], "agent_result": None}
        finally:
            pool.shutdown(wait=False)

        return [r for r in results if r is not None]

    def _run_agent_step_blocking(self, project, step_index, step, cfg, lang,
                                 context_msg, out_q):
        """Worker-thread body of one parallel agent step. NEVER touches the
        trace, total_usage or yields — it only streams the sub-agent, pushes
        displayable events to the queue and finishes with a 'done' record.
        Mirrors _execute_agent_step minus the live text relay (parallel mode
        is multi-step, where text is written by the synthesis)."""
        t0 = time.perf_counter()
        sub_trace, answer_parts, status = None, [], "ok"
        agent_result, error_txt = None, None
        try:
            completion = project.get_llm(cfg["agent_id"]).new_completion()
            if context_msg and cfg.get("pass_context"):
                completion.with_message(context_msg, role="system")
            ...
            for chunk in completion.execute_streamed():
                ...
                if ctype == "event":
                    ...
                    human_label = _sub_event_label(sub_kind, ed, lang, cfg)
                    if human_label is None:
                        continue            # technical event -> masked
                    ed["agentKey"] = step["capability"]
                    ed["stepIndex"] = step_index
                    ed["label"] = human_label
                    out_q.put(("event", step_index,
                               {"chunk": {"type": "event", "eventKind": sub_kind,
                                          "eventData": ed}}))
                elif ctype in ("content", "text"):
                    txt = data.get("text", "") or ""
                    if txt:
            ...
            logger.exception("Parallel agent step %d failed (%s)",
                             step_index, step["capability"])
            status = "error"
            error_txt = str(e)[:500]
        ...
        res = {
            "kind": "agent", "capability": step["capability"],
            "instruction": step["instruction"], "status": status,
            "output": answer,
            "duration_ms": int((time.perf_counter() - t0) * 1000),
            "usage": {}, "generated_sql": [], "agent_result": agent_result,
        }
        out_q.put(("done", step_index, {"res": res, "sub_trace": sub_trace,
                                        "error": error_txt}))
