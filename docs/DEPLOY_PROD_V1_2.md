...
> Carte des ids agents/tools/modèles : `OWIsMind_PRD_V1_3_DEV/README.md` + `OWIsMind_PRD_V1_3_DEV/registry.json`.
>
...
est `OWIsMind_PRD_V1_3_DEV/` (le vieux dossier `dataiku-agents/` n'existe plus).

## 1. Plugin (instance) - v1.2.0, id `owismind`

1. **Builder le frontend** via le skill **`/build-plugin`** (`npm run build` →
   `OWIsMind_PRD_V1_3_DEV/plugin/owismind/resource/owismind-app/`, puis recopie `index.html` → `body.html`). NO-INSTALL : si
   `node_modules` manque, s'arrêter et demander à l'utilisateur (l'agent n'installe jamais).
2. **Packager** via le skill **`/package-plugin`** → `OWIsMind_PRD_V1_3_DEV/plugin/ready-for-dataiku/owismind-v1_2-upload.zip`
...
Source de vérité = **`OWIsMind_PRD_V1_3_DEV/GenAI/Agents/*.py`** (miroir verbatim de ce qui est collé dans DSS) :

| Fichier repo | Objet DSS | id |
|---|---|---|
| `OWIsMind_PRD_V1_3_DEV/GenAI/Agents/OWIsMind_orchestrator.py` | Code Agent orchestrateur | `038G7mlF` |
| `OWIsMind_PRD_V1_3_DEV/GenAI/Agents/SalesDrive_revenue_expert.py` | Code Agent sous-agent revenus | `bHrWLyOL` (ref `agent:bHrWLyOL`) |
| `OWIsMind_PRD_V1_3_DEV/GenAI/Agents/CSSO_Trouble_Tickets_Expert.py` | Code Agent sous-agent tickets | `NcE9LD2i` (ref `agent:NcE9LD2i`) |
| `OWIsMind_PRD_V1_3_DEV/GenAI/agents-tools/attribute_lookup_tool.py` | Custom Python tool `attribute_lookup_tool` | `UUoynaL` |
...
`OWIsMind_PRD_V1_3_DEV/README.md` + `registry.json` + `docs/PLAYBOOK_ADD_AGENT.md`.
...
notebook du clone** (`OWIsMind_PRD_V1_3_DEV/GenAI/semantic-models/scripts/`).
...
- **Tickets** (`TroubleTickets_Semantic_Model` = `dM4jA4G`) : **repoint EN ATTENTE**. Script prêt et
  simulé (4/4) : `OWIsMind_PRD_V1_3_DEV/GenAI/semantic-models/scripts/repoint_tickets_prod_clone.py`. À lancer
...
- Une **branche git par version**, nommée d'après le projet DSS de prod. `OWIsMind_PRD_V1_2` = branche de
  ...
  `OWIsMind_PRD_V1_3_DEV/plugin/tools/build_dev_plugin.py`, zip `owismind-v1_3-dev-upload.zip` une fois son `plugin.json` en `1.3.x`).
