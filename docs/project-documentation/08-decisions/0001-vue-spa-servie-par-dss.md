...
The source code lives in `OWIsMind_PRD_V1_3_DEV/plugin/owismind/frontend/` (Composition API, ES modules). The dependencies in
...
`OWIsMind_PRD_V1_3_DEV/plugin/owismind/frontend/vite.config.js` sets two canonical values that must never be renamed:
...
`OWIsMind_PRD_V1_3_DEV/plugin/owismind/webapps/webapp-owismind-ai-agents/body.html` as the page body. After each build, the
