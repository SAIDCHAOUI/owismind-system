...
|---|---|---|
| Vue 3 SPA (frontend) | `OWIsMind_PRD_V1_3_DEV/plugin/owismind/frontend/`, built into static assets in `OWIsMind_PRD_V1_3_DEV/plugin/owismind/resource/owismind-app/` | Renders the three screen areas (conversations sidebar, chat + timeline, Evidence Studio), sends opaque logical keys to the backend and displays the evidence. |
| Flask backend (python-lib) | `OWIsMind_PRD_V1_3_DEV/plugin/owismind/python-lib/owismind/`, Python 3.9.23 env | A Flask Blueprint mounted under `/owismind-api` that resolves identity, applies the agent whitelist, runs the agent in a worker, persists in direct SQL and re-derives Evidence. |
