...
`OWIsMind_PRD_V1_3_DEV/plugin/owismind/plugin.json`), webapp `webapp-owismind-ai-agents`, python-lib package `owismind`, resource
...
The frontend is a Vue 3 SPA under `OWIsMind_PRD_V1_3_DEV/plugin/owismind/frontend/`. The
`OWIsMind_PRD_V1_3_DEV/plugin/owismind/frontend/package.json` file pins the versions (`dependencies` / `devDependencies` fields):
...
(`OWIsMind_PRD_V1_3_DEV/plugin/owismind/frontend/vite.config.js`) fits in a few lines and fixes two canonical names,
...
The backend is a Flask module under `OWIsMind_PRD_V1_3_DEV/plugin/owismind/python-lib/owismind/`. The runtime observed in DSS
...
restated in `OWIsMind_PRD_V1_3_DEV/plugin/owismind/python-lib/CLAUDE.md`: "observed backend is 3.9.23 (Flask)".
...
The entry point is thin: `OWIsMind_PRD_V1_3_DEV/plugin/owismind/webapps/webapp-owismind-ai-agents/backend.py` merely
...
- Frontend: `node --test test/*.test.js` (the native `node:test` runner, not Vitest). Pure tests under
  `OWIsMind_PRD_V1_3_DEV/plugin/owismind/frontend/test/`, outside `src/`, so never built or zipped.
- Backend: `python3 -m unittest discover -s OWIsMind_PRD_V1_3_DEV/plugin/owismind/tests`. Tests outside `python-lib/`, so never
  packaged.
