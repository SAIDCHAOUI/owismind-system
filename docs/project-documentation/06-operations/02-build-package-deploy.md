...
> `python3 OWIsMind_PRD_V1_3_DEV/plugin/tools/build_dev_plugin.py` / the `/package-plugin-dev` skill, so you can install it alongside
...
1. **Preflight**: verify that `OWIsMind_PRD_V1_3_DEV/plugin/owismind/frontend/node_modules/` exists. If it is missing, STOP: only
   the user installs (NO INSTALL policy). The install command would in any case be refused by the harness.
2. **Build** from the repository root: `npm --prefix OWIsMind_PRD_V1_3_DEV/plugin/owismind/frontend run build`. The `build` script
...
`OWIsMind_PRD_V1_3_DEV/plugin/owismind/frontend/vite.config.js` sets three values that must never change without a rebuild and a
rewiring of `body.html`:

| Vite key | Value | Role |
|---|---|---|
| `base` | `/plugins/owismind/resource/owismind-app/` | URL prefix of the assets served by DSS |
| `build.outDir` | `../resource/owismind-app` | output (relative to `frontend/`, so `OWIsMind_PRD_V1_3_DEV/plugin/owismind/resource/owismind-app`) |
...
1. **Reset the staging**: `rm -rf` of the `OWIsMind_PRD_V1_3_DEV/plugin/ready-for-dataiku/owismind-upload` folder and the zip,
...
2. **Stage the runtime**: `plugin.json` goes to the ROOT of the staging; `python-lib/`, `resource/` and
   ...
   `OWIsMind_PRD_V1_3_DEV/plugin/owismind/plugin.json`, there is no `_/plugin.json` in this repository.
...
| `OWIsMind_PRD_V1_3_DEV/plugin/ready-for-dataiku/**` (the deliverable zip) | ignored | regenerated by `/package-plugin` |
...
| Suite | Command | Note |
|---|---|---|
| Backend | `python3 -m unittest discover -s OWIsMind_PRD_V1_3_DEV/plugin/owismind/tests -v` | folder outside `python-lib/`, never packaged |
| Frontend | `npm --prefix OWIsMind_PRD_V1_3_DEV/plugin/owismind/frontend test` | runner `node --test test/*.test.js`, without Vue or dataiku |
