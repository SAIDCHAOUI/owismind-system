...
File: `OWIsMind_PRD_V1_3_DEV/plugin/owismind/frontend/vite.config.js`. Three settings carry the entire deployment
contract:
...
| `build.outDir` | `../resource/owismind-app` | Build target, relative to `frontend/`. It therefore points to `OWIsMind_PRD_V1_3_DEV/plugin/owismind/resource/owismind-app/`, sibling of the `frontend/` folder. |
...
`OWIsMind_PRD_V1_3_DEV/plugin/owismind/frontend/CLAUDE.md` (section "Build & output") and in the `/build-plugin` skill. The
...
The source entry point is `OWIsMind_PRD_V1_3_DEV/plugin/owismind/frontend/index.html`. It is deliberately minimal:
...
`OWIsMind_PRD_V1_3_DEV/plugin/owismind/frontend/public/`: Vite copies it as-is into the output.

## 4. The build output: `resource/owismind-app/`

`npm run build` (alias for `vite build`) writes into `OWIsMind_PRD_V1_3_DEV/plugin/owismind/resource/owismind-app/`:
...
ignored" philosophy; the deliverable zip (`OWIsMind_PRD_V1_3_DEV/plugin/ready-for-dataiku/`) itself remains ignored.
...
`OWIsMind_PRD_V1_3_DEV/plugin/owismind/frontend/node_modules`:
