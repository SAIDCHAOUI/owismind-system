...
``OWIsMind_PRD_V1_3_DEV/GenAI/Agents/SalesDrive_revenue_expert.py``).
...
class TestModePropagation(unittest.TestCase):
    """The sub-agent mirrors the orchestrator's mode (smart=mini, pro=Gemini,
    claude=Sonnet), read from the injected context the orchestrator builds."""

    def test_forced_mode_parses_injected_context(self):
        ctx = "MODE: claude\nUSER LANGUAGE: fr - write any message…"
        self.assertEqual(dx.forced_mode(ctx), "claude")
        self.assertEqual(dx.forced_mode("MODE: smart\n…"), "smart")
    ...
    def test_pick_subagent_llm_per_mode(self):
        self.assertEqual(dx.pick_subagent_llm("smart"), dx.GEMINI_FLASH_LITE_ID)
        self.assertEqual(dx.pick_subagent_llm("pro"), dx.GEMINI_FLASH_ID)
        self.assertEqual(dx.pick_subagent_llm("claude"), dx.SONNET_ID)

    def test_semantic_tool_id_per_mode_defaults_shared(self):
        # All modes share the one tool id until a Sonnet-backed tool exists for claude.
        self.assertEqual(dx.pick_semantic_tool_id("smart"), dx.SEMANTIC_TOOL_ID)
        self.assertEqual(dx.pick_semantic_tool_id("claude"), dx.SEMANTIC_TOOL_ID)
