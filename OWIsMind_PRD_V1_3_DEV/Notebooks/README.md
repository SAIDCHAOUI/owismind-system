...
Ils pilotent le package `owismind_factory` et le Config & Prompt Hub `/python/owismind_hub/`.
...
| `01_push_config_hub.py` | Pousse les seeds du Config & Prompt Hub `/python/owismind_hub/` (settings, capabilities.json, persona, template engine) | Seeds equivalentes aux defauts embarques : ne change rien au comportement ; n'ecrit que ce qui est absent sauf `FORCE = True` |
