...
| Source Data v3 (Run 2 : backdrop popover, mesures choisies `calcFns`, plages sur colonnes string, cascade distinct POST, persistance des vues `sourceViewMemory` ; Run 3 : bornes MIMANT le format des valeurs L138, « Filtrer sur cette valeur » depuis la cellule) | ⏳ Validé local (revues soldées + QA), zip dev_v2 `index-gYAOS4OQ.js` à uploader | - | L134-L138 (`sessions/2026-07-06.md` Runs 2-3) |
| Contexte écran -> agent (bandeau consentement, `screen_context.source_state` -> section SOURCE-DATA VIEW du bloc `[ON SCREEN NOW]`, events 41 ; Run 3 : TRANSPARENCE : colonne `screen_ctx` sur chat_v5 + chip cliquable + ligne persistée sous le message) | ⏳ Validé local ; zip dev_v2 + restart, paragraphe orchestrateur DEV à recoller (env 3.11) | - | L134/L136/L138 (`sessions/2026-07-06.md` Runs 2-3) |
