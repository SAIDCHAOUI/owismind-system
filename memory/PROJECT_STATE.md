...
| **Audit agents + langue/UX/escalade (2026-06-16 Run 4)** : **langue** (détection word-boundary du dernier prompt, bloc contexte appendé EN FIN, token `⟦owi:lang⟧` autoritaire lu en DERNIER = anti-spoof, sous-agent `forced_language`) ; **messages live = vrais** (préambule modèle via `_txt`/answer_delta persisté, pas NARRATION) ; **fix mini narrate-and-stop** (prompt **ACT-FIRST** + nudge + **AUTO-escalade Sonnet** sur échec) ; **escalade pilotée par le modèle** (`LoopChat` transcript-replay + `escalate_to_expert` + `switch_model` rôles alternés + verrou one-way ; `pick_loop_llm` simplifié, `complexity_is_high` retiré) ; **conscience écran** (`build_screen_state`/`_build_screen_block` owner-scopé gaté panneau ouvert + `screen_context` frontend) ; **timeline UX** (`timelineSegments` ignore narration → max-5 globale + gris/orange + hiérarchie ; event `ESCALATING`) | ⏳ **CODÉ + revues OK, NON validé DSS**. **Recoller LES 2 Code Agents** (env 3.11) + upload zip + redémarrer backend | 179 agents + 381 backend + 114 node verts ; audit 7-dim + revue adversariale (**1 bloqueur 400 Vertex corrigé**) + revue sécu Dataiku **« safe-to-ship »** ; zip **77 entrées `index-CT0YQv_u.js`** ; **L066-L070** ; ⚠️ rôles alternés obligatoires sur escalade (sinon 400 Vertex) |
