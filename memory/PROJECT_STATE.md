...
| **Modèle sémantique aligné + sous-agent ASSISTIF** (NOUVEAU modèle sémantique sur Sonnet 4.6, l'ancien `2O2KcHw` intact ; scripts `dataiku-agents/semantic_model/{build,update}_aligned_semantic_model.py` + `README.md` ; corrections ACTUAL→ACTUALS/glossaire/hiérarchie offre/affichage nom+carrier/Account_partner/une-table-no-join/9 golden queries ; `build_semantic_question` refondu : question user + HINTS, **terme d'offre ambigu NON épinglé → le modèle tranche**) | ✅ Nouveau modèle **créé + indexé + tool `v4oqA6R` repointé + Sonnet** (user, Playground OK). ⏳ **fix sous-agent codé + 157 tests verts, à RE-COLLER (`agent:AKQaQ0Am`, env 3.11) + re-tester EVPL via l'orchestrateur** | bug réglé : pin déterministe `sirano_product='EVPL'`→budget=0 (`column_priority` fallback `-distinct_count`) ; `create_semantic_model` + workflow versions, update en place sans re-index, jamais instancier les classes (L058-L059) ; **id du nouveau modèle à consigner** |
