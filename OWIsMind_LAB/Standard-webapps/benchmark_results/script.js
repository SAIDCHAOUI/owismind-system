...
(function () {
  ...
  var DICT = {
    ...
    "ae.h": { en: "What the agent actually did", fr: "Ce que l'agent a reellement fait" },
    "ae.answer": { en: "Full agent answer", fr: "Reponse complete de l'agent" },
    "ae.sql.h": { en: "SQL generated by the agent", fr: "SQL genere par l'agent" },
    "ae.query": { en: "Query {n}", fr: "Requete {n}" },
    "ae.rows.n": { en: "{n} row(s)", fr: "{n} ligne(s)" },
    "ae.ok": { en: "success", fr: "succes" },
    "ae.failed": { en: "failed", fr: "echec" },
    "ae.data": { en: "Data used to answer", fr: "Donnees utilisees pour repondre" },
    "ae.noSql": { en: "The agent generated no SQL for this answer.", fr: "L'agent n'a genere aucun SQL pour cette reponse." },
    "ae.noData": { en: "No result rows were captured for this query.", fr: "Aucune ligne de resultat n'a ete capturee pour cette requete." },
    "ae.truncated": { en: "Result truncated for display.", fr: "Resultat tronque pour l'affichage." },
    "ae.loading": { en: "Loading the full detail...", fr: "Chargement du detail complet..." },
    "ae.error": { en: "Could not load the full detail.", fr: "Impossible de charger le detail complet." },
    "ae.empty": { en: "No captured detail for this attempt.", fr: "Aucun detail capture pour cette tentative." },
  ...
  function MOCK(path) {
    ...
    if (clean === "api/results/attempt") { return mockAttempt(path); }
    return { status: "ok" };
  }

  /* Offline preview of the on-demand full detail: a complete answer + generated SQL + result table,
     varied by the question_id in the query string so different rows show different evidence. */
  function mockAttempt(path) {
    var q = (path.split("question_id=")[1] || "").split("&")[0];
    if (q === "q_rev_02") {
      return {
        status: "ok", found: true, run_id: "run_2026-06-29_22-14", question_id: q,
        agent_key: "038G7mlF", mode: "Smart", status_row: "ok", actual_tools: "chart,table", n_sql: 1, total_rows: 5,
        answer_text: "The top 5 customers year to date, by revenue, are Airbus, Maroc Telecom, Orange, Vodafone and MTN. Airbus leads with EUR 1.28bn, ahead of Maroc Telecom (EUR 0.94bn). The ranking is stable versus last quarter.",
        sql_items: [
          { sql: "SELECT customer, SUM(amount_eur) AS rev\nFROM drive_revenues\nGROUP BY customer\nORDER BY rev DESC\nLIMIT 5", success: true, row_count: 5,
            result: { columns: ["customer", "rev"], truncated: false,
              rows: [["Airbus", 1284300000], ["Maroc Telecom", 938400000], ["Orange", 812900000], ["Vodafone", 655100000], ["MTN", 511200000]] } }
        ]
      };
    }
    if (q === "q_tic_01") {
      return {
        status: "ok", found: true, run_id: "run_2026-06-29_22-14", question_id: q,
        agent_key: "038G7mlF", mode: "Pro", status_row: "ok", actual_tools: "", n_sql: 1, total_rows: 1,
        answer_text: "Airbus currently has 47 open trouble tickets across all services.",
        sql_items: [
          { sql: "SELECT COUNT(*) AS open_tickets\nFROM trouble_tickets\nWHERE account = 'Airbus' AND status = 'OPEN'", success: true, row_count: 1,
            result: { columns: ["open_tickets"], truncated: false, rows: [[47]] } }
        ]
      };
    }
    return {
      status: "ok", found: true, run_id: "run_2026-06-29_22-14", question_id: q || "q_rev_01",
      agent_key: "038G7mlF", mode: "Claude", status_row: "ok", actual_tools: "table", n_sql: 1, total_rows: 1,
      answer_text: "The total actual revenue for 2025 was EUR 1.28 billion (1,284,300,000), all scenarios actuals.",
      sql_items: [
        { sql: "SELECT SUM(amount_eur) AS total_revenue\nFROM drive_revenues\nWHERE phase = 'ACTUALS' AND year = 2025", success: true, row_count: 1,
          result: { columns: ["total_revenue"], truncated: false, rows: [[1284300000]] } }
      ]
    };
  }
  ...
  /* Lazy shell for the FULL agent detail (complete answer + generated SQL + result table). Filled on
     first expand via /api/results/attempt so the heavy blobs load one row at a time (never en masse). */
  function agentEvidenceShell(r) {
    return '<div class="agent-ev" data-loaded="0"' +
      ' data-run="' + esc(r.run_id || "") + '" data-q="' + esc(r.question_id || "") + '"' +
      ' data-ak="' + esc(r.agent_key || "") + '" data-mode="' + esc(r.mode || "") + '">' +
      '<div class="ae-head">' + esc(t("ae.h")) + '</div>' +
      '<div class="ae-body"><p class="ae-msg">' + esc(t("ae.loading")) + '</p></div>' +
    '</div>';
  }

  /* Render the loaded full-detail payload into an evidence shell body. */
  function agentEvidenceBody(d) {
    if (!d || !d.found) { return '<p class="ae-msg">' + esc(t("ae.empty")) + '</p>'; }
    var answer = d.answer_text ? esc(d.answer_text) : esc(t("det.noAns"));
    var html = '<div class="ae-answer"><div class="ae-l">' + esc(t("ae.answer")) + '</div>' +
      '<pre class="ae-pre">' + answer + '</pre></div>';
    var items = d.sql_items || [];
    if (!items.length) { return html + '<p class="ae-msg">' + esc(t("ae.noSql")) + '</p>'; }
    html += '<div class="ae-l ae-sql-h">' + esc(t("ae.sql.h")) + '</div>';
    items.forEach(function (it, i) { html += sqlItemHtml(it, i + 1); });
    return html;
  }

  /* One generated SQL query: the SQL text + success/row-count + the captured result table. */
  function sqlItemHtml(it, n) {
    var okCls = it.success ? "ae-ok" : "ae-bad";
    var okLabel = it.success ? t("ae.ok") : t("ae.failed");
    var rc = it.row_count != null ? it.row_count : ((it.result && it.result.rows) ? it.result.rows.length : 0);
    var meta = '<span class="ae-badge ' + okCls + '">' + esc(okLabel) + '</span>' +
      '<span class="ae-rc">' + esc(t("ae.rows.n", { n: fmtInt(rc) })) + '</span>';
    var sql = it.sql ? '<pre class="ae-sql">' + esc(it.sql) + '</pre>' : "";
    return '<div class="ae-item">' +
      '<div class="ae-item-head"><span class="ae-qn">' + esc(t("ae.query", { n: n })) + '</span>' + meta + '</div>' +
      sql + resultTableHtml(it.result) +
    '</div>';
  }

  /* The captured result table (columns + rows), scrollable, with an honest truncation note. */
  function resultTableHtml(res) {
    if (!res || !res.columns || !res.columns.length) {
      return '<div class="ae-data"><div class="ae-l">' + esc(t("ae.data")) + '</div>' +
        '<p class="ae-msg">' + esc(t("ae.noData")) + '</p></div>';
    }
    var head = res.columns.map(function (c) { return '<th>' + esc(c) + '</th>'; }).join("");
    var body = (res.rows || []).map(function (row) {
      return '<tr>' + (row || []).map(function (cell) {
        return '<td>' + esc(cell == null ? "" : cell) + '</td>';
      }).join("") + '</tr>';
    }).join("");
    var trunc = res.truncated ? '<p class="ae-msg ae-trunc">' + esc(t("ae.truncated")) + '</p>' : "";
    var table = (res.rows && res.rows.length)
      ? '<div class="ae-table-wrap"><table class="ae-table"><thead><tr>' + head + '</tr></thead><tbody>' + body + '</tbody></table></div>'
      : '<p class="ae-msg">' + esc(t("ae.noData")) + '</p>';
    return '<div class="ae-data"><div class="ae-l">' + esc(t("ae.data")) + '</div>' + table + trunc + '</div>';
  }

  /* Fetch + render the full detail for one attempt, once, when its row is first expanded. */
  function loadAgentEvidence(box) {
    if (!box || box.getAttribute("data-loaded") !== "0") { return; }
    box.setAttribute("data-loaded", "1");
    var body = box.querySelector(".ae-body");
    if (body) { body.innerHTML = '<p class="ae-msg">' + esc(t("ae.loading")) + '</p>'; }
    var path = "api/results/attempt?run_id=" + encodeURIComponent(box.getAttribute("data-run") || "") +
      "&question_id=" + encodeURIComponent(box.getAttribute("data-q") || "") +
      "&agent_key=" + encodeURIComponent(box.getAttribute("data-ak") || "") +
      "&mode=" + encodeURIComponent(box.getAttribute("data-mode") || "");
    api(path).then(function (d) {
      if (body) { body.innerHTML = agentEvidenceBody(d); }
    }, function () {
      box.setAttribute("data-loaded", "0"); // allow a retry on the next expand
      if (body) { body.innerHTML = '<p class="ae-msg ae-err">' + esc(t("ae.error")) + '</p>'; }
    });
  }
  ...
  function wireContent() {
    ...
    qsa('[data-r="toggle"]', c).forEach(function (btn) {
      btn.addEventListener("click", function () {
        ...
        // Lazily load the full agent detail (complete answer + generated SQL + result table) on first open.
        if (open) { loadAgentEvidence(detail.querySelector(".agent-ev")); }
      });
    });
  }
