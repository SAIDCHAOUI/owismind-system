...
(function () {
  ...
  var DICT = {
    ...
    "rv.answer.show": { en: "Show agent answer + SQL + data", fr: "Voir la reponse + SQL + donnees" },
    "rv.answer.hide": { en: "Hide agent answer + SQL + data", fr: "Masquer la reponse + SQL + donnees" },
    "rv.answer.empty": { en: "(no answer captured)", fr: "(aucune reponse capturee)" },
    "rv.ae.answer": { en: "Full agent answer", fr: "Reponse complete de l'agent" },
    "rv.ae.sqlH": { en: "SQL generated by the agent", fr: "SQL genere par l'agent" },
    "rv.ae.query": { en: "Query {n}", fr: "Requete {n}" },
    "rv.ae.rows": { en: "{n} row(s)", fr: "{n} ligne(s)" },
    "rv.ae.ok": { en: "success", fr: "succes" },
    "rv.ae.failed": { en: "failed", fr: "echec" },
    "rv.ae.data": { en: "Data used to answer", fr: "Donnees utilisees pour repondre" },
    "rv.ae.noSql": { en: "The agent generated no SQL for this answer.", fr: "L'agent n'a genere aucun SQL pour cette reponse." },
    "rv.ae.noData": { en: "No result rows were captured for this query.", fr: "Aucune ligne de resultat n'a ete capturee pour cette requete." },
    "rv.ae.truncated": { en: "Result truncated for display.", fr: "Resultat tronque pour l'affichage." },
    "rv.ae.loading": { en: "Loading the full detail...", fr: "Chargement du detail complet..." },
    "rv.ae.error": { en: "Could not load the full detail.", fr: "Impossible de charger le detail complet." },
    "rv.ae.empty": { en: "No captured detail for this attempt.", fr: "Aucun detail capture pour cette tentative." },
  ...
  /* Offline preview of the on-demand attempt full detail (complete answer + generated SQL + table),
     varied by question_id so different review rows show different evidence. */
  function mockReviewAttempt(params) {
    var q = params.question_id || "";
    if (q === "a_revenue001") {
      return {
        status: "ok", found: true, run_id: params.run_id || "", question_id: q,
        agent_key: "038G7mlF", mode: params.mode || "Smart", actual_tools: "table", n_sql: 1, total_rows: 1,
        answer_text: "Le revenu reel (ACTUALS) du compte Maroc Telecom est de 4 218 540 EUR sur l'annee en cours, toutes periodes confondues.",
        sql_items: [
          { sql: "SELECT SUM(amount_eur) AS revenue\nFROM drive_revenues\nWHERE account_name = 'Maroc Telecom' AND phase = 'ACTUALS'", success: true, row_count: 1,
            result: { columns: ["revenue"], truncated: false, rows: [[4218540]] } }
        ]
      };
    }
    if (q === "u_sug_d4e5f6") {
      return {
        status: "ok", found: true, run_id: params.run_id || "", question_id: q,
        agent_key: "038G7mlF", mode: params.mode || "Smart", actual_tools: "table", n_sql: 1, total_rows: 1,
        answer_text: "Algerie Telecom currently has 41 open trouble tickets.\n\nNote: this counted raw snapshot rows, not distinct ticket ids.",
        sql_items: [
          { sql: "SELECT COUNT(*) AS open_tickets\nFROM trouble_tickets\nWHERE account = 'Algerie Telecom' AND status = 'OPEN'", success: true, row_count: 1,
            result: { columns: ["open_tickets"], truncated: false, rows: [[41]] } }
        ]
      };
    }
  ...
  function mockApi(method, path, body) {
    ...
    } else if (method === "GET" && path0 === "review/attempt") {
      data = mockReviewAttempt(parseQuery(path));
  ...
  function reviewCardHtml(r) {
    ...
    var answer = '<div class="rvw-answer">' +
      ...
      (expanded ? reviewDetailHtml(rk, r) : '') +
    '</div>';
  ...
  /* The rich detail shown under a review row when expanded: the FULL agent answer + the SQL the agent
     actually generated + each query's captured result table. Loaded on demand into S.review.detail[rk]
     so the reviewer decides the override WITH full context (not a truncated preview). */
  function reviewDetailHtml(rk, r) {
    var d = S.review.detail[rk];
    if (!d || d.loading) { return '<p class="rvw-ae-msg">' + esc(t("rv.ae.loading")) + '</p>'; }
    if (d.error) { return '<p class="rvw-ae-msg rvw-ae-err">' + esc(t("rv.ae.error")) + '</p>'; }
    var data = d.data || {};
    if (!data.found) {
      // No captured full detail: fall back to the light preview the review list already carries.
      var prev = (r && r.answer_preview != null) ? String(r.answer_preview) : "";
      return '<pre class="rvw-pre">' + esc(prev || t("rv.ae.empty")) + '</pre>';
    }
    var answer = data.answer_text ? esc(data.answer_text) : esc(t("rv.answer.empty"));
    var html = '<div class="rvw-ae">' +
      '<div class="rvw-ae-l">' + esc(t("rv.ae.answer")) + '</div>' +
      '<pre class="rvw-pre">' + answer + '</pre>';
    var items = data.sql_items || [];
    if (!items.length) {
      html += '<p class="rvw-ae-msg">' + esc(t("rv.ae.noSql")) + '</p>';
      return html + '</div>';
    }
    html += '<div class="rvw-ae-l rvw-ae-sqlh">' + esc(t("rv.ae.sqlH")) + '</div>';
    items.forEach(function (it, i) { html += rvSqlItemHtml(it, i + 1); });
    return html + '</div>';
  }

  function rvIntStr(n) { var v = Number(n); return isFinite(v) ? String(Math.round(v)) : "0"; }

  function rvSqlItemHtml(it, n) {
    var okCls = it.success ? "rvw-ae-ok" : "rvw-ae-bad";
    var okLabel = it.success ? t("rv.ae.ok") : t("rv.ae.failed");
    var rc = (it.row_count != null) ? it.row_count : ((it.result && it.result.rows) ? it.result.rows.length : 0);
    var meta = '<span class="rvw-ae-badge ' + okCls + '">' + esc(okLabel) + '</span>' +
      '<span class="rvw-ae-rc">' + esc(t("rv.ae.rows", { n: rvIntStr(rc) })) + '</span>';
    var sql = it.sql ? '<pre class="rvw-ae-sql">' + esc(it.sql) + '</pre>' : "";
    return '<div class="rvw-ae-item">' +
      '<div class="rvw-ae-ihead"><span class="rvw-ae-qn">' + esc(t("rv.ae.query", { n: n })) + '</span>' + meta + '</div>' +
      sql + rvResultTableHtml(it.result) +
    '</div>';
  }

  function rvResultTableHtml(res) {
    if (!res || !res.columns || !res.columns.length) {
      return '<div class="rvw-ae-data"><div class="rvw-ae-l">' + esc(t("rv.ae.data")) + '</div>' +
        '<p class="rvw-ae-msg">' + esc(t("rv.ae.noData")) + '</p></div>';
    }
    ...
    var trunc = res.truncated ? '<p class="rvw-ae-msg rvw-ae-trunc">' + esc(t("rv.ae.truncated")) + '</p>' : "";
    var table = (res.rows && res.rows.length)
      ? '<div class="rvw-ae-twrap"><table class="rvw-ae-table"><thead><tr>' + head + '</tr></thead><tbody>' + body + '</tbody></table></div>'
      : '<p class="rvw-ae-msg">' + esc(t("rv.ae.noData")) + '</p>';
    return '<div class="rvw-ae-data"><div class="rvw-ae-l">' + esc(t("rv.ae.data")) + '</div>' + table + trunc + '</div>';
  }

  /* Fetch the full detail of ONE attempt once, when its review row is first expanded. */
  function loadReviewAttempt(row) {
    if (!row) { return; }
    var rk = reviewRowKey(row);
    if (S.review.detail[rk]) { return; }  // already loading / loaded
    S.review.detail[rk] = { loading: true };
    var path = "review/attempt?run_id=" + encodeURIComponent(row.run_id || "") +
      "&question_id=" + encodeURIComponent(row.question_id || "") +
      "&agent_key=" + encodeURIComponent(row.agent_key || "") +
      "&mode=" + encodeURIComponent(row.mode || "");
    callApi("GET", path).then(function (res) {
      var d = res.data || {};
      S.review.detail[rk] = (d.status === "ok") ? { data: d } : { error: true };
      renderReview();
    }, function () {
      S.review.detail[rk] = { error: true };
      renderReview();
    });
  }
  ...
  function wireReview() {
    ...
    qsa("[data-rv]", box).forEach(function (el) {
      ...
      el.addEventListener("click", function () {
        ...
        if (kind === "toggle") {
          var nowOpen = !S.review.expanded[rk];
          S.review.expanded[rk] = nowOpen;
          // Lazily load the full detail (complete answer + generated SQL + result table) on first open.
          if (nowOpen) { loadReviewAttempt(findReviewRow(rk)); }
          renderReview();
          return;
        }
