...
> admins/operators). Last updated: 2026-06-18. Summary: this portal introduces OWIsMind in
...
|---|---|---|
| **User** (analyst, salesperson, manager) | [Getting started](01-user-guide/01-getting-started.md) | [Using the chat](01-user-guide/02-using-the-chat.md), [Understanding the results](01-user-guide/03-understanding-evidence.md), [FAQ and troubleshooting](01-user-guide/04-faq-and-troubleshooting.md) |
...
| [`01-user-guide/`](01-user-guide/01-getting-started.md) | User guide: getting started, using the chat, reading the results (Evidence), FAQ. |
...
| [`08-decisions/`](08-decisions/README.md) | Architecture decisions (ADR): the 12 structuring choices and their rationale. |
...
## Points in flux (as of 2026-06-18)

The agent layer (`dataiku-agents/`) is currently being edited. A few elements are in transition and are
documented as such on the relevant pages:

> IN FLUX: the `attribute_lookup` tool (`dataiku-agents/tools/attribute_lookup_tool.py`) is BUILT and
> unit-tested, but NOT yet wired into the sub-agent. Its predecessor, the managed `dataset_lookup`
> tool (`9FEzVZk`) and the `lookup` intent, were REMOVED on 2026-06-18. Attribute lookups
> (for example the account_manager of an account) are therefore in transition.

> ROADMAP: `DRIVE_Revenues_Value_Catalog` and the Python resolver `Drive_Revenues_resolve_filter_value`
> are NOT wired in v3. The monthly budget quota (50 EUR/user/month) has its STORAGE ready
> (`webapp_usage_monthly_v1`) but the BLOCKING is NOT implemented. The per-mode LLM Mesh ids
> (`GEMINI_FLASH_LITE_ID`, `GEMINI_FLASH_ID`, `SONNET_ID`) must match the instance connection;
> a wrong id breaks the corresponding mode.
